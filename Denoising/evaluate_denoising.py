...
try:
    ...
    print("Error: Could not import MPRNet. Make sure evaluate_denoising.py is inside the 'Denoising' folder.")
    exit()
...
def main():
    ...
    # Evaluate SIDD locally 
    datasets = ['SIDD'] 
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Initializing Denoising Evaluation on: {device.type.upper()}")
    ...
    with torch.no_grad():
        for dataset_name in datasets:
            ...
            for sub_in, sub_tgt in subfolders_to_try:
                inp_path = os.path.join(base_dataset_dir, dataset_name, sub_in, 'input')
                tgt_path = os.path.join(base_dataset_dir, dataset_name, sub_tgt, 'target')
                if os.path.exists(inp_path) and os.path.exists(tgt_path):
                    if len(glob(os.path.join(inp_path, '*.*'))) > 0:
                        input_dir = inp_path
                        target_dir = tgt_path
                        break
            
            if input_dir is None or target_dir is None:
                print(f"Skipping {dataset_name}: Could not locate any valid 'input' and 'target' image pairs in standard subdirectories.")
                continue
    ...
    csv_filename = "evaluate_denoising.csv"
