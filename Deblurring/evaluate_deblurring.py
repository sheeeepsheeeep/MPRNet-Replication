...
# Import MPRNet architecture 
try:
    from MPRNet import MPRNet
except ImportError:
    print("Error: Could not import MPRNet. Make sure evaluate.py is inside the 'Deblurring' folder.")
    exit()
...
def main():
    ...
    print(f"Initializing Evaluation on: {device.type.upper()}")
    ...
    with torch.no_grad():
        for dataset_name in datasets:
            ...
            input_dir = os.path.join(base_dataset_dir, dataset_name, 'test', 'input')
            target_dir = os.path.join(base_dataset_dir, dataset_name, 'test', 'target')
            ...
            for inp_path in tqdm(input_paths, desc=f"Processing {dataset_name}"):
                ...
                # Forward pass - use Mixed Precision (FP16) only if on CUDA
                if device.type == 'cuda':
                ...
                # Force PyTorch to release the memory back to the GPU after every image
                del input_tensor
                if device.type == 'cuda':
    ...
    csv_filename = 'deblurring_results.csv'
    print(f"\nExporting results to {csv_filename}...")
    
    with open(csv_filename, mode='w', newline='') as file:
        writer = csv.writer(file)
        # Write the header row
        writer.writerow(['Dataset', 'PSNR (dB)', 'SSIM'])
